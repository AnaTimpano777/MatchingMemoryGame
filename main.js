...
$(document).ready(function() {
    ...
    if(localStorage.score[localStorage.name] != undefined) {
        score = localStorage.score[localStorage.name];
        $('#score').text(score);
    }
    $('#field img').on({
    'click': function(){
        ...
        setTimeout(function() {  
                    $('.flipped').not('[src="images/silhouette.png"]').attr('src','images/back.png');
                },3500); 
...
function checkWin() {
    if($('#field IMG').not('[src="images/silhouette.png"]').length == 0) {  
        ...
        score++;
        localStorage.score[localStorage.name] = score;
        $('#score').text(score);
        setTimeout(function() {
...
$('#field IMG').each(function(counter) {
  ...
  $('#reset-button').click(function() {
        if(confirm('If you confirm this message your score will reset!')) {
    resetGame();
        }
    });
function resetGame() {
